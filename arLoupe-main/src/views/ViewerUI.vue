        ...
        <div class="lg:col-span-8 flex flex-col gap-6">
          ...
          <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div class="space-y-4">
              ...
              <div class="mt-4 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-300 dark:border-slate-600">
              <label class="text-[10px] font-black text-slate-500 uppercase block mb-2">上傳新影片</label>
              <input type="file" @change="handleUpload" accept="video/*" class="text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer w-full" />
            </div>
            </div>
          </div>
        </div>
...
const handleUpload = async (event) => {
  ...
  formData.append('title', file.name); // 對應 Django 的 title 欄位
  formData.append('file', file);       // 對應 Django 的 file 欄位

  try {
    const res = await axios.post('http://127.0.0.1:8000/api/videos/', formData);
    console.log('上傳成功！');
    
    // 重點：上傳成功後，立刻叫 Store 去後端抓最新清單
    await recordingsStore.fetchRecordings(); 
    
    alert('影片上傳成功，清單已更新！');
  } catch (err) {
    console.error('上傳出錯', err);
    alert('上傳失敗，請檢查後端是否啟動');
  }
};
