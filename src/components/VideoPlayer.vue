      ...
      <div class="p-4 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-4">
            ...
            <button @click="toggleOverlayMode" class="ml-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-200 rounded-lg text-[11px] font-black transition-all flex items-center gap-1.5 border border-slate-200 dark:border-slate-600 shadow-sm active:scale-95">
              ...
              {{ overlayMode === 'off' ? '檢視臨床影像' : (overlayMode === 'xray' ? '切換至根尖片' : '關閉影像比對') }}
            </button>
          </div>
        </div>
        <div class="flex items-center gap-3">
...
const toggleOverlayMode = () => {
    if (overlayMode.value === 'off') overlayMode.value = 'xray';
    else if (overlayMode.value === 'xray') overlayMode.value = 'apical';
    else overlayMode.value = 'off';
};
